...
## Argo CD GitOps Provider — Task A6: argocd-bootstrap.yml + docs (2026-07-07)
- `kubectl kustomize` writes to stdout. When validating with `> /dev/null 2>&1 && echo OK || echo FAIL` in a for loop, a stale terminal buffer can show YAML output BEFORE the redirect takes effect — making it look like FAIL when exit code is actually 0. Always check exit code directly: `kubectl kustomize <path> > /dev/null 2>&1; echo "exit: $?"` rather than relying on the `&&/||` chain output display alone.
- GitHub Actions `workflow_dispatch` `type: choice` inputs restrict values to the declared `options:` list — using them via `env: PROVIDER: ${{ inputs.provider }}` then `${PROVIDER}` in shell is the safe pattern (avoids command injection even though choice inputs are actor-controlled). The `name:` label field using `${{ inputs.* }}` is a display string only, not a shell execution context.
- When appending a new `### Added` block to an existing CHANGELOG `## [Unreleased]` section that already has content, extend the existing `### Added` block rather than adding a duplicate `### Added` heading — two `### Added` headings in the same `## [Unreleased]` section is malformed Keep a Changelog.
- The `gitops/README.md` arrow notation rule confirmed: use `->` not `→` throughout (Unicode arrow does not render in all Markdown viewers; `->` is already the established convention in this file).
- `azure/setup-kubectl` SHA `3e0aec4d80787158d308d7b364cb1381784a5f79` pins to v4 — confirmed from the plan. Matches the SHA-pinning convention established in `flux-bootstrap.yml` (actions/checkout `11bd71901bbe5b1630ceea73d27597364c9af683`).

## Argo CD GitOps Provider — Task A4: Argo Rollouts + blast-radius AnalysisTemplate (2026-07-07)
- Downloaded `argo-rollouts-install-v1.7.0.yaml` is 16770 lines — vendor this file (commit to git) for air-gapped or reproducible builds. The plan correctly notes the curl download command for initial setup.
- Argo Rollouts `AnalysisTemplate` uses `provider.web.url` with `{{args.flag-key}}` templating for dynamic flag key injection — the `args:` list in the AnalysisTemplate defines the parameter names, and they are bound per-Rollout in the `analysis.args` steps.
- The blast-radius web provider uses GET by default (no `method:` field needed) because there is no request body — the flag key is passed as a URL query parameter `?flag_key={{args.flag-key}}`. The evaluator endpoint is at port 8082 (not 8081 which is flag-api).
- `successCondition` and `failureCondition` both reference `result.risk_score` where `result` is the parsed JSON response body (via `jsonPath: "{$}"`). LOW and MEDIUM allow canary promotion; HIGH and BLOCKED abort with `failureLimit: 1` (first failure stops the rollout immediately).
- The `gitops/providers/both/kustomization.yaml` already referenced `rollouts.yaml` from Task A1 — no update needed in Task A4. Verify with `grep rollouts gitops/providers/both/kustomization.yaml` before adding to avoid duplication.
- `rollout-analysis.yaml` lives in `gitops/apps/production/tombstone/` alongside the Helm chart's HelmRelease — it is an application-layer concern, not a cluster-bootstrap concern. The Rollout replaces the standard Deployment for flag-api only when Argo Rollouts is active (argocd|both provider).

## Argo CD GitOps Provider — Task A3: Argo CD Application manifests (2026-07-07)
- `grep -c "ignoreDifferences"` counts ALL occurrences including comment lines — the plan's "Expected: 2" refers to the number of Applications that have the key, not the raw grep count. Use `grep -c "^  ignoreDifferences:"` (with leading spaces for YAML indentation) to count actual YAML keys vs. comment mentions. Both tombstone-app and tombstone-flags have the key; tombstone-root intentionally does NOT (App-of-Apps, not managing FeatureFlag resources).
- The plan's exact file contents include both `spec.ignoreDifferences` AND `spec.syncPolicy.syncOptions: [RespectIgnoreDifferences=true]` on `tombstone-app` and `tombstone-flags` but NOT on `tombstone-root` — tombstone-root is the App-of-Apps that owns Application objects (not FeatureFlag/FlagEnvironment CRs), so it does not need ML rollout protection.
- Staging apps.yaml omits `tombstone-root` — only production has the App-of-Apps root. Staging gets direct Application objects for tombstone-app and tombstone-flags pointing at `targetRevision: develop` and their respective staging paths.
- The `directory.include: "apps-children.yaml"` filter on tombstone-root references a file that does not exist yet — this is intentional (App-of-Apps pattern where children are defined separately). Argo CD will show tombstone-root as OutOfSync until apps-children.yaml is created, which is expected at this stage.
