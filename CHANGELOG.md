...
- **Live `targeting_rules_updated` streaming — all 5 SDKs + backend** (#245–#251): the `targeting_rules` table — scaffolded since the baseline schema but never wired to any endpoint, snapshot field, or event — now has a full CRUD API (`POST/GET/DELETE /flags/{key}/environments/{env}/rules[/{id}]`), snapshot embedding, and its own per-environment-scoped live SSE event, using the same discriminator design as `prerequisites_updated` (#245). TypeScript (#246), Java (#247, #250), Ruby (#248), .NET (#249), and Python (#251) all consume it live, each proactively applying the tie-break/one-shot-provenance lesson chain `prerequisites_updated` had already taught this session (a `>=` staleness guard comparing cached `targetingRulesUpdatedAt` against the event's `ts`, gated by a one-shot live-event-provenance flag that the next snapshot load always resets; malformed/non-object SSE payloads swallowed, not raised; an event for an unknown `flagKey` is a no-op), plus a per-SDK wire-shape adapter converting flag-api's flat per-rule wire row into each SDK's richer internal `TargetingRule` model.
- **EVAL-4 — HALF_OPEN recovery loop + stepped auto-rollback** (#220, #221): the circuit breaker's auto-rollback is no longer an instant 100%→0% kill — it now walks a deterministic `100→50→25→0` ladder via a new `POST /flags/{key}/rollback-step` endpoint (`flags:circuit_breaker` permission, service-token-only role), and recovers via a real `HALF_OPEN` probe loop (`10→25→50→100`, backed by a new `recovery-step` endpoint) that reverts to `OPEN`/0% if any probe step's error rate is still bad, rather than a bare 10-minute TTL reopening full traffic with zero verification. The manual `/api/v1/rollback` endpoint now requires bearer auth for the first time.
- **EVAL-3 — real telemetry persistence + blast radius data** (#222): the circuit breaker and telemetry aggregator now actually write the hourly Redis keys the SLO endpoint has always read from (previously all-zero). Blast radius calculation is rewritten on top of it: real evaluation volume/error rate (replacing a hardcoded fake constant), the real `flag_prerequisites` dependency graph (replacing an audit-log co-change heuristic), owner-based affected-services, and a `Confidence`/cold-start label.
- **EVAL-2 — real SDK telemetry + Slack alerting** (#218, #219): `@tombstone/core`'s `TombstoneClient` now buffers a telemetry event per `evaluate()` call and flushes to the evaluator's `/api/v1/telemetry` endpoint on an interval (opt-in via a new `telemetryUrl` config field — zero behavior change for existing callers) — closing the gap where the circuit-breaker pillar had never once fired from real production traffic. A circuit-breaker auto-rollback now also posts a Slack alert with a dashboard deep-link, deduplicated across racing evaluator replicas.
...
- **SDK prerequisite gating never worked against a real backend** (#227–#233): flag-api's `prerequisites` wire field was named `prereq_flag_key`, diverging from the `flag_key` every SDK's type was actually written against (#228, the rename). Independently, Python, Java, Ruby, and .NET's snapshot-fetch code never parsed the `prerequisites` array from a real response at all (defaulting to empty), and several SDKs' `evaluate()` never threaded a real flag lookup through to the prerequisite-resolution step — compounding into prerequisite gating being completely inert client-side regardless of what flag-api actually sent. Python's OFF-path also never consulted a disabled dependency's own `safe_default`. All fixed per-SDK, plus a live cache-wiping regression (#227) where Python's and Java's event-apply paths rebuilt the cached flag state from only the event's own fields, silently wiping `prerequisites`/`targeting_rules`/`target_list` on every real flag-toggle SSE event until the next snapshot refetch.
- **SDK-3 conformance harness gaps** (#226): Python's hash-parity test suite never actually loaded `test-contract/vectors.json` — its coverage was 4 hand-copied literal tuples that would silently not follow if the shared vectors file were ever corrected. Now loads and parametrizes against the real file, matching every other SDK. `target_list_vectors` (Step 3 coverage) had zero consumers in any SDK; wired into Java's contract test as the first real one.
- **marketplace — Datadog auto-kill-switch was silently dead** (#223): the alert-triggered auto-remediation flow 401'd on every call (missing `Authorization` header), decoded flag-api's response with the wrong shape, and queried a nonexistent `tag` filter and blast-radius route — any one of the three alone kept it from ever firing, even for a genuinely `BLOCKED` flag.
- **EXP-2 — mSPRT e-value formula error** (#214): `analyze_sequential`/`analyze_sequential_from_stats` subtracted a spurious extra term with no basis in the method's own mixture-likelihood-ratio derivation, making true positives converge toward non-significance as sample size grew — the opposite of mSPRT's intended always-valid power (e.g. a case that should read `e=269.15` was instead reading `e=0.090`, never significant).
- **Java SDK REGEX operator raised instead of returning `false`** (#250), diverging from `docs/SDK_CONTRACT.md`'s documented cross-SDK contract (already correct in TS/Ruby/.NET/Python) — a gap in Java's own #247, found by .NET's review of #249.
- **Python SDK — two pre-existing bugs found while adding targeting_rules streaming, not regressions introduced by it** (#251): (1) `_apply_snapshot` had no tie-break/monotonicity protection at all, for prerequisites or targeting_rules — any out-of-order snapshot could silently regress already-applied live-event state; retrofitted the same one-shot-provenance guard the other 4 SDKs already had. (2) the targeting_rules wire adapter assumed a nested `"conditions"` key per rule that does not exist on flag-api's real, flat wire shape (`id`/`attribute`/`operator`/`values`/`variation`/`priority` on one row) — an existing test encoded the identical wrong shape as its own fixture, masking the bug; both the adapter and the test are now corrected.
