...
export class EvaluationEngine {
  ...
  evaluateWithDetail<T>(
    ...
    return this.evaluateInternal<T>(
      flagKey,
      context,
      defaultValue,
      cache,
  ...
  evaluate<T>(
    ...
    if (typeof flagStateOrKey === "string") {
      ...
      const rules = (
        Array.isArray(flagKeyOrRules) ? flagKeyOrRules : []
      ) as TargetingRule[];
      return this.evaluateWithDetail<T>(
        flagKey,
        context,
        defaultVal,
        cache,
    ...
    const flagKey = (
      typeof flagKeyOrRules === "string" ? flagKeyOrRules : ""
    ) as string;
  ...
  private evaluateInternal<T>(
    ...
    if (!flagState.enabled) {
      return this.result(
        this.parseSafeDefault<T>(flagState.safeDefault, defaultValue),
        "OFF",
        flagKey,
      );
    }
    ...
    if (prereqs.length > 0 && depth < EvaluationEngine.MAX_PREREQ_DEPTH) {
      const blocked = this.checkPrerequisites<T>(
        prereqs,
        context,
        defaultValue,
        cache,
    ...
    const sortedRules = [...(flagState.targetingRules ?? []), ...rules].sort(
    ...
    if (
      this.isInRollout(
        flagKey,
        userId,
        flagState.rolloutPct,
        flagState.hashVersion ?? 1,
      )
    ) {
  ...
  private checkPrerequisites<T>(
    ...
    for (const prereq of prereqs) {
      ...
      if (!prereqState && prereq.gate) {
        return this.result(defaultValue, "PREREQUISITE_FAILED", parentKey, {
  ...
  private applyOperator(
    ...
    switch (operator) {
      ...
      // NOT_IN with an EMPTY ruleValues must NOT match: Array.prototype.every
      // is vacuously true on an empty array, so a rule with an empty/missing
      // "values" list (a malformed write past validation, or a live
      // targeting_rules_updated/snapshot payload that defensively defaults a
      // missing wire "values" field to []) would otherwise match EVERY
      // context for EVERY attribute -- the exact opposite of "exclude
      // nothing configured, so exclude nothing". Found by adversarial review
      // of PR #246. ruleValues.length > 0 restores the safe default: no
      // exclusion list configured means this rule never matches via NOT_IN.
      case "NOT_IN":
        return (
          ruleValues.length > 0 &&
          ruleValues.every((v) => String(v) !== strValue)
        );
      case "EQ":
  ...
  private result<T>(
    ...
    return {
      ...
      ...(extra?.variationIndex !== undefined
        ? { variationIndex: extra.variationIndex }
        : {}),
    };
  }
