...
const activeFlag = (
  pct: number,
  overrides: Partial<FlagEnvironmentState> = {},
...
const singleCache = (
  flag: FlagEnvironmentState | undefined,
  flagKey = "test-flag",
) => ({
...
describe("EvaluationEngine — legacy signature (flagState, rules, ctx, default, key)", () => {
  ...
  it("TARGET_MATCH: dot-notation attribute (geo.country)", () => {
    const geoCtx: EvaluationContext = {
      userId: "user-geo",
      geo: { country: "US" },
    };
    const result = engine.evaluate(
  ...
  it("TARGET_MATCH: GTE operator on numeric attribute", () => {
    const numCtx: EvaluationContext = {
      userId: "user-num",
      attrs: { score: "95" },
    };
    const result = engine.evaluate(
      ...
      [
        {
          id: "r5",
          ruleType: "CUSTOM",
          attribute: "nonexistent.deep.key",
          operator: "EQ",
          values: ["x"],
          variation: "v",
          priority: 5,
...
describe("Step 1 — Preliminary", () => {
  it("ERROR when flag not in cache", () => {
    const r = engine.evaluateWithDetail("missing", ctx, false, {
      get: () => undefined,
    });
  ...
  it("OFF when flag is disabled, returns safeDefault", () => {
    ...
    const r = engine.evaluateWithDetail(
      "test-flag",
      ctx,
      true,
      singleCache(flag),
...
describe("Step 2 — Prerequisites", () => {
  it("PREREQUISITE_FAILED when gating prereq is disabled", () => {
    const prereqFlag = activeFlag(100, {
      flagKey: "prereq-flag",
      enabled: false,
      safeDefault: "false",
    });
    const mainFlag = activeFlag(100, {
      flagKey: "main-flag",
      prerequisites: [
        { flagKey: "prereq-flag", requiredVariation: "true", gate: true },
      ],
    });
    const cache = multiCache({
      "main-flag": mainFlag,
      "prereq-flag": prereqFlag,
    });
  ...
  it("PREREQUISITE_FAILED when prereq resolves to wrong variation", () => {
    ...
    const mainFlag = activeFlag(100, {
      flagKey: "main-flag",
      prerequisites: [
        { flagKey: "prereq-flag", requiredVariation: "enabled", gate: true },
      ],
    });
    const cache = multiCache({
      "main-flag": mainFlag,
  ...
  it("non-gating prereq failure skips rule but continues evaluation", () => {
    ...
    const mainFlag = activeFlag(100, {
      flagKey: "main-flag",
      prerequisites: [
        { flagKey: "prereq-flag", requiredVariation: "true", gate: false },
      ],
    });
    const cache = multiCache({
      "main-flag": mainFlag,
  ...
  it("missing prereq flag with gate=true → PREREQUISITE_FAILED", () => {
    const mainFlag = activeFlag(100, {
      flagKey: "main-flag",
      prerequisites: [
        { flagKey: "ghost-flag", requiredVariation: "true", gate: true },
      ],
    });
...
describe("Step 4 — Rule matching", () => {
  ...
  it("RULE_MATCH: PREFIX operator", () => {
    const ctxPrefix: EvaluationContext = {
      userId: "user-abc",
      attrs: { email: "admin@acme.com" },
    };
    const flag = activeFlag(100);
    const r = engine.evaluateWithDetail(
      "test-flag",
  ...
  it("RULE_MATCH: NOT_IN operator excludes a listed value", () => {
    const ctxBanned: EvaluationContext = {
      userId: "user-abc",
      attrs: { plan: "banned" },
    };
    const flag = activeFlag(100);
    ...
    assert.equal(
      r.reason,
      "FALLTHROUGH",
      "a listed (excluded) value must NOT match NOT_IN",
    );
  });

  it("RULE_MATCH: NOT_IN operator matches a value not in the list", () => {
    const ctxOk: EvaluationContext = {
      userId: "user-abc",
      attrs: { plan: "pro" },
    };
    const flag = activeFlag(100);
  ...
  it("NOT_IN with an EMPTY values list never matches -- an empty exclusion list must not vacuously match every context", () => {
    // Array.prototype.every is vacuously true on an empty array -- without
    // an explicit length check, a rule with an empty/missing "values" list
    // (a malformed write, or a wire payload defensively defaulting a
    // missing "values" field to []) would match EVERY context for EVERY
    // attribute, the exact opposite of "no exclusions configured, so
    // exclude nothing". Found by adversarial review of PR #246.
    const flag = activeFlag(100);
    const r = engine.evaluateWithDetail(
      "test-flag",
      ctx,
...
describe("Step 5 — Fallthrough rollout", () => {
  ...
  it("MurmurHash stickiness at 50%", () => {
    ...
    assert.ok(
      allSame,
      "consistent hashing: same user must always land in same bucket",
    );
  });
});
