...
//
// NAMING COLLISION NOTE (evaluator only): this service also has
// internal/circuit, a Redis-backed CLOSED/OPEN/HALF_OPEN state machine that
// drives automatic FLAG rollback business logic (see internal/circuit/breaker.go).
// That breaker answers "is this feature flag causing too many errors in
// production, and should we roll it back?" — it is keyed per-flag and its
// state is shared across evaluator replicas via Redis.
//
// The circuit breaker built into THIS package (via failsafe-go's
// circuitbreaker.Builder below) is a completely different, in-process-only
// breaker that protects a single outbound HTTP call — e.g. evaluator's own
// call to flag-api's kill switch in internal/rollback. It answers "is the
// HTTP call to flag-api itself currently failing, so should we stop hammering
// it?" It has no Redis state and no awareness of flags. The two are unrelated
// concepts that happen to both be called "circuit breaker"; keep them in
// separate packages (already true here) and do not conflate them.
package httpclient
...
func NewResilientClient(cfg ResilientClientConfig, httpClient *http.Client, logger *zap.Logger) *ResilientClient {
	...
	// caller's trace — this client never touched Transport at all, so
	// inbound requests' spans (every service already creates one via
	// otelhttp.NewHandler) died at the first inter-service hop. Wraps
	// whatever is ALREADY on httpClient.Transport (a caller-supplied mTLS
	// transport, or nil) as otelhttp.NewTransport's base — nil is handled
	// by otelhttp itself (defaults to http.DefaultTransport), so this is
	// safe regardless of which case applies, and never discards a
	// caller-supplied custom transport.
