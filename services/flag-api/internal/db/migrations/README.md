...
| 023 | `023_targeted_indexes.sql` | DATA-2: five indexes for existing hot-path query patterns that had no supporting index — `flag_environments(environment)`, `change_requests(status, created_at)`, `flags(project_id, state)`, `audit_log(created_at)`, `scim_users(email)`. Purely additive, no application code change. |
