    ...
    def get_degree_data(self) -> dict[str, dict]:
        ...
        for flag in all_flags:
            ...
            result[flag] = {
                ...
                "avg_edge_weight": sum(all_weights) / len(all_weights)
                if all_weights
                else 0.0,
            }
        return result
    ...
    async def fetch(self, query: str, *args):
        ...
        if "audit_log" in query:
            ...
            for i, (source, target, expected_weight) in enumerate(self.edges):
                ...
                delta = min(
                    delta, COUPLING_WINDOW_SECONDS - 10
                )  # Keep under the coupling window
...
def test_critical_flags_endpoint_returns_valid_structure(app_with_critical_flags_mock):
    ...
    app_with_critical_flags_mock.state.graph_builder._get_pool = AsyncMock(
...
def test_critical_flags_evaluator_unreachable_fallback_low(
...
@patch("app.main.httpx.AsyncClient.get")
def test_critical_flags_scoring_formula_a_b_c_d_ranking(
    mock_evaluator_get, app_with_critical_flags_mock
):
    """
    ...
    def mock_evaluator_call(url: str, **kwargs):
        ...
        return SimpleNamespace(
            status_code=200, json=lambda t=tier: {"result": {"risk_score": t}}
        )
    ...
    assert flags[1]["key"] == "B", (
        f"Expected second flag to be B, got {flags[1]['key']}"
    )
    assert flags[2]["key"] == "C", f"Expected third flag to be C, got {flags[2]['key']}"
    assert flags[3]["key"] == "D", (
        f"Expected fourth flag to be D, got {flags[3]['key']}"
    )
    ...
    assert flags[0]["in_degree"] == 1, (
        f"A in_degree should be 1, got {flags[0]['in_degree']}"
    )
    assert flags[0]["out_degree"] == 2, (
        f"A out_degree should be 2, got {flags[0]['out_degree']}"
    )

    # B: in=1 (A->B), out=1 (B->C)
    assert flags[1]["in_degree"] == 1, (
        f"B in_degree should be 1, got {flags[1]['in_degree']}"
    )
    assert flags[1]["out_degree"] == 1, (
        f"B out_degree should be 1, got {flags[1]['out_degree']}"
    )

    # C: in=3 (A->C, B->C, D->C), out=0
    assert flags[2]["in_degree"] == 3, (
        f"C in_degree should be 3, got {flags[2]['in_degree']}"
    )
    assert flags[2]["out_degree"] == 0, (
        f"C out_degree should be 0, got {flags[2]['out_degree']}"
    )

    # D: in=0, out=2 (D->A, D->C)
    assert flags[3]["in_degree"] == 0, (
        f"D in_degree should be 0, got {flags[3]['in_degree']}"
    )
    assert flags[3]["out_degree"] == 2, (
        f"D out_degree should be 2, got {flags[3]['out_degree']}"
    )
    ...
    # A: (1+2)*0.7727*4 ≈ 9.27
    assert 9.0 < flags[0]["score"] < 9.5, (
        f"A score should be ~9.27, got {flags[0]['score']}"
    )

    # B: (1+1)*0.7506*3 ≈ 4.50
    assert 4.4 < flags[1]["score"] < 4.7, (
        f"B score should be ~4.50, got {flags[1]['score']}"
    )

    # C: (3+0)*0.6445*2 ≈ 3.87
    assert 3.7 < flags[2]["score"] < 4.0, (
        f"C score should be ~3.87, got {flags[2]['score']}"
    )
