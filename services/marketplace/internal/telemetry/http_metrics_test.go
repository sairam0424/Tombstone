...
func TestHTTPMetrics_RecordsRouteMethodAndStatusClass(t *testing.T) {
	...
	r := chi.NewRouter()
	r.Use(mw)
	r.Get("/api/v1/circuit/{flagKey}", func(w http.ResponseWriter, req *http.Request) {
		w.WriteHeader(http.StatusNotFound)
	})

	req := httptest.NewRequest(http.MethodGet, "/api/v1/circuit/should-not-appear-in-labels", nil)
	...
	for _, sm := range got.ScopeMetrics {
		for _, m := range sm.Metrics {
			...
			if !ok || route.AsString() != "/api/v1/circuit/{flagKey}" {
				t.Errorf("route attribute = %v, want the chi PATTERN, not the raw path", route)
			}
...
func TestHTTPMetrics_PanicRecordedAndRePanicked(t *testing.T) {
	...
	if route := routeAttr(t, &got); route != "/health" {
		t.Errorf("route = %q, want /health — the route pattern was already resolved before the panic, so it should still be captured", route)
	}
	for _, sm := range got.ScopeMetrics {
		for _, m := range sm.Metrics {
			if m.Name != "http_server_requests_total" {
