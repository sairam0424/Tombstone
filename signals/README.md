...
## Tombstone Signal Domains

Signals in this repo map to the following domain loops:

- `flag-cleanup` — stale flags, zombie configs, removal friction
- `incident-response` — blast-radius events, rollback triggers, postmortem patterns
- `rollout-advisor` — experiment collisions, bandit convergence, SLO drift
- `governance` — approval workflow gaps, OPA policy pain, audit-log requests
