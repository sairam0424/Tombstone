...
// The shell has no environment switcher yet — every page that fetches its
// own state independently picks "production" as its own default (e.g.
// FlagDetail's own activeEnv). ConnectionStatus/LiveFeed need SOME env to
// watch at the shell level (outside any specific page), so this matches
// that same existing implicit assumption rather than inventing a new
// global env-selector as a side effect of just wiring these two
// components in.
const SHELL_ENVIRONMENT = "production";
...
export default function App() {
  return (
    ...
    >
      {/* ── Sidebar ────────────────────────────────────── */}
      <aside
        className="w-56 flex-shrink-0 flex flex-col sidebar-glow overflow-y-auto"
              ...
              <ul className="space-y-0.5">
                {section.items.map((item) => (
                  <li key={item.to}>
                    ...
                    >
                      <span className="text-base w-4 text-center leading-none">
                        {item.icon}
                      </span>
                      {item.label}
                    </NavLink>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <ConnectionStatus env={SHELL_ENVIRONMENT} />
          </div>
        </header>

        {/* Page content + live feed */}
        <div className="flex-1 flex overflow-hidden">
          ...
          <LiveFeed env={SHELL_ENVIRONMENT} />
        </div>
      </div>
    </div>
  );
