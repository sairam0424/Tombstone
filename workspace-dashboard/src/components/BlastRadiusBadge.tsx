  ...
  return (
    <div className={`rounded-lg border p-4 ${cfg.color}`}>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs mb-3">
        <div className="bg-black/20 rounded p-2">
          ...
          <div className="text-white font-medium mt-0.5">
            {result.traffic_pct_affected.toFixed(0)}%
          </div>
        </div>
        <div className="bg-black/20 rounded p-2">
          <div className="text-gray-400">Dependent flags</div>
          <div className="text-white font-medium mt-0.5">
            {result.dependent_flags_count}
          </div>
        </div>
        <div className="bg-black/20 rounded p-2">
        ...
        <div className="bg-black/20 rounded p-2">
          <div className="text-gray-400">
            ...
            {result.confidence === "LOW" && (
              <span className="text-amber-400"> (cold start)</span>
            )}
          </div>
          {result.confidence === "LOW" ? (
            <div className="font-medium mt-0.5 text-gray-500">unmeasured</div>
          ) : (
            <div
              className={`font-medium mt-0.5 ${result.historical_error_rate > 0.03 ? "text-red-300" : "text-white"}`}
            >
              {(result.historical_error_rate * 100).toFixed(1)}%
            </div>
          )}
        </div>
        <div className="bg-black/20 rounded p-2 col-span-2">
          <div className="text-gray-400">Evaluations (24h)</div>
          <div className="text-white font-medium mt-0.5">
            {result.recent_evaluation_count.toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        <button
          onClick={() =>
            onConfirm(needsJustification ? justification : undefined)
          }
