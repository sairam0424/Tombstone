...
export default function FlagDetail() {
  ...
  const [checkingBlastRadius, setCheckingBlastRadius] = useState(false);
  const [pendingKillEnv, setPendingKillEnv] = useState<Env | null>(null);
  const [blastRadius, setBlastRadius] = useState<BlastRadiusResult | null>(
    null,
  );
  ...
  const killSwitch = async (env: Env, justification?: string) => {
    if (!key) return;
    setKilling(true);
    try {
  ...
  // switch specifically (rollout_pct=0, the actual effect of disabling)
  // rather than gating every action, since it's the single highest-stakes
  // button already on this page.
  //
  ...
  // precedent for this exact service.
  const requestKillSwitch = async (env: Env) => {
    if (!key) return;
    setCheckingBlastRadius(true);
    try {
      const res = await fetch(
        `${EVAL_URL}/api/v1/blast-radius?flag_key=${encodeURIComponent(key)}&environment=${env}&rollout_pct=0`,
      );
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = (await res.json()) as { result?: BlastRadiusResult };
      if (!data.result) throw new Error("no result in response");
      setBlastRadius(data.result);
      setPendingKillEnv(env);
    } catch {
      // Fail open — proceed with the kill immediately.
      await killSwitch(env);
    } finally {
      setCheckingBlastRadius(false);
    }
  };

  const confirmKill = (justification?: string) => {
    const env = pendingKillEnv;
    setBlastRadius(null);
    setPendingKillEnv(null);
    if (env) void killSwitch(env, justification);
  };
          ...
          <div className="bg-gray-900 rounded-lg border border-gray-700 p-5 mb-6">
            ...
            {envState?.enabled && (
              <div className="mt-4 pt-4 border-t border-gray-700">
                {blastRadius && pendingKillEnv === activeEnv ? (
                  <BlastRadiusBadge
                    result={blastRadius}
                  ...
                  <>
                    <button
                      ...
                      disabled={killing || checkingBlastRadius}
