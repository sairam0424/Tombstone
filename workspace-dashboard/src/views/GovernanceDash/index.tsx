...
interface StaleFlag {
  ...
  // null = ast-rewriter unreachable/unconfigured, not "zero references" --
  // the backend never recommends ARCHIVE when this is null (INT-6).
...
function StaleFlagsTable({
  ...
  if (stale.length === 0) {
    return (
      <EmptyRow message="No stale flags detected — flag hygiene looks good" />
    );
  }
      ...
      >
        ...
        <tbody>
          {stale.map((f) => {
            ...
            const as =
              ACTION_STYLE[f.recommended_action] ?? ACTION_STYLE["REVIEW"];
            ...
            return (
              ...
              >
                ...
                <td style={{ padding: "11px 16px" }}>
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      color:
                        f.days_at_100_pct >= 60
                          ? T.red
                          : f.days_at_100_pct >= 30
                            ? T.amber
                            : T.textPrimary,
                      fontVariantNumeric: "tabular-nums",
                    }}
                ...
                {/* Code references (INT-6) -- null means ast-rewriter
                    couldn't verify (unreachable/unconfigured), NOT zero;
                    the backend never recommends ARCHIVE on a null value. */}
                <td style={{ padding: "11px 16px" }}>
                  <span
                    style={{
                      fontSize: 12,
                    ...
                    title={
                      f.call_site_count === null
                        ? "ast-rewriter could not verify live code references"
                        : `${f.recent_evaluation_count} recent evaluations`
                    }
                  >
                    {f.call_site_count === null ? "unknown" : f.call_site_count}
                  </span>
                </td>
      ...
      >
        ...
        <tbody>
          {recs.map((rec) => {
            ...
            const confColor =
              confPct >= 80 ? T.green : confPct >= 60 ? T.amber : T.red;
            const envColor =
        ...
        return (
          <div
            ...
            style={{
              ...
              padding: "10px 20px",
              borderBottom:
                idx < entries.length - 1 ? `1px solid ${T.borderSub}` : "none",
            }}
          >
            {/* dot + connecting line */}
            <div
...
export default function GovernanceDash() {
  ...
  const { data: staleFlagsData = [], isLoading: staleLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // called a nonexistent /api/v1/intelligence/stale-flags path and
  ...
  const { data: activityData = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data.entries ?? []).map((e) => ({
        flag_key:
          (e["flag_key"] as string) ?? (e["entity_id"] as string) ?? "—",
  ...
  const handleApplyRec = async (rec: AutonomousRecommendation) => {
    ...
    try {
      ...
      toast.error("Failed to apply recommendation", {
        description:
          err instanceof Error ? err.message : "Rollout update failed",
      });
    } finally {
      setApplyingKey(null);
    }
      ...
      {health && !loading && (
        <>
          ...
          <Reveal delay={0}>
            ...
            >
              ...
              <StatCard
                ...
                sub={
                  health.stale_flags === 0
                    ? "None detected"
                    : `${health.stale_flags} flag${health.stale_flags !== 1 ? "s" : ""} need review`
                }
              />
          ...
          >
            ...
            <Reveal delay={0.24}>
              <SectionCard title="Recent Activity" subtitle="Last 7 days">
